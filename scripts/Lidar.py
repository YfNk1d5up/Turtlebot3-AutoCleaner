...
def checkCrash(lidar):
    ...
    W = np.append(W, np.linspace(1, 1.2, len(lidar_horizon) // 2+1 ))
    print("min cracsh",np.min( W * lidar_horizon ))
    print(W)
    print(lidar_horizon)
    if np.min( W[lidar_horizon > 0] * lidar_horizon[lidar_horizon > 0] ) < COLLISION_DISTANCE:
        return True
    else:
        return False
...
def checkObjectNearby(lidar):
    ...
    W = np.append(W, np.linspace(1, 1.4, len(lidar_horizon) // 2+1))
    if np.min( W[lidar_horizon > 0] * lidar_horizon[lidar_horizon > 0] ) < NEARBY_DISTANCE:
        return True
    else:
        return False
